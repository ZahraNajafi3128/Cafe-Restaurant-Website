...
const VAT_RATE      = 0.025;      // ۲.۵٪ نمونه
const SHIPPING_COST = 100000;     // هزینه ثابت ارسال
...
// به‌روزرسانی آیکون دکمه minus بر اساس تعداد
function updateMinusIcon(item) {
  ...
  if (qty <= 1) {
    minusEl.setAttribute('data-icon', 'trash'); // دایره سفید + سطل
    minusEl.textContent = "";                   // متن - مخفی
  } else {
    minusEl.setAttribute('data-icon', 'minus'); // دایره مشکی با -
    minusEl.textContent = "−";
  }
}
...
// حذف همه سفارش‌ها با کلیک روی دکمه بالای کارت
if (clearAllBtn) {
...
// تنظیم اولیه آیکون minus برای همه آیتم‌ها و محاسبه مبالغ
